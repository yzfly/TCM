...
class ResNet(nn.Module):
    ...
    def __init__(self, block, layers, num_segments, enable_TCM, num_classes=1000, zero_init_residual=False):
        super(ResNet, self).__init__()
        self.num_segments = num_segments     
        self.enable_TCM = enable_TCM
        ...
        if enable_TCM:
            self.tcm_layer = TCM(self.num_segments, expansion = block.expansion, pos=2)
...
def resnet18(pretrained=False, shift='TSM',num_segments = 8, enable_TCM=0, **kwargs):
    ...
    """
    if (shift =='TSM'):    
        model = ResNet(BasicBlock, [2, 2, 2, 2], num_segments=num_segments , enable_TCM=enable_TCM, **kwargs)  
    if pretrained:
...
def resnet34(pretrained=False, shift='TSM',num_segments = 8, enable_TCM=0,**kwargs):
    ...
    """
    if (shift =='TSM'):
        model = ResNet(BasicBlock, [3, 4, 6, 3],num_segments=num_segments , enable_TCM=enable_TCM,  **kwargs)        
    if pretrained:
...
def resnet50(pretrained=False, shift='TSM', num_segments = 8, enable_TCM=0, **kwargs):
    ...
    """
    if (shift =='TSM'):    
        model = ResNet(Bottleneck, [3, 4, 6, 3],num_segments=num_segments , enable_TCM=enable_TCM, **kwargs)          
    if pretrained:
...
def resnet101(pretrained=False, shift='TSM',num_segments = 8, enable_TCM=0, **kwargs):
    ...
    """
    if (shift =='TSM'):    
        model = ResNet(Bottleneck, [3, 4, 23, 3],num_segments=num_segments , enable_TCM=enable_TCM, **kwargs)          
    if pretrained:
